...
import { ChevronRight, ChevronDown, Table, BarChart3, RefreshCw, AlertTriangle, CheckCircle, FolderOpen, Cloud, Zap, Filter, Users, Download } from 'lucide-react';
...
const DurationInput = ({ work, onConfirm, disabled }) => {
...
function SchedulingTab({ docId, currentObj, api, lang = 'ru', t = {}, projectNameOverride = '', objectNameOverride = '' }, ref) {
  ...
  const ganttBlockRef = useRef(null); // ссылка на отрисованную диаграмму Ганта — для экспорта в PDF
  ...
  const fetchScheduleData = async () => {
    ...
    setErrorToast('');
    try {
      const res = await api.get(`/estimates/schedule/${docId}?lang=${lang}`);
      const payload = res.data || {};
      setProjectData(payload.project || currentObj?.project || null);
      setDocMeta(payload.doc || null);
  ...
  // Строим самодостаточную статическую SVG-диаграмму Ганта для печати — не клонируем
  // живой интерактивный DOM (он завязан на скролл-контейнеры фиксированной ширины и
  // при отсоединении от страницы рендерился пустой белой областью в PDF).
  const buildStaticGanttSvg = (worksList) => {
    ...
    const dayOffset = (dateObj) => Math.round((dateObj - minDate) / (1000 * 60 * 60 * 24));
    const rowY = {};
    worksList.forEach((w, idx) => { rowY[w.id] = headerH + idx * rowH + rowH / 2; });

    const monthBandH = showDayTicks ? 18 : headerH;

    // Заголовок по месяцам
    let monthsHtml = '';
    let curMonth = -1, curX = leftW, curSpanDays = 0, curLabel = '';
    const daysArr = [];
    ...
    daysArr.forEach((d, i) => {
      ...
      if (m !== curMonth) {
        if (curMonth !== -1) {
          monthsHtml += `<rect x="${curX}" y="0" width="${curSpanDays * dayW}" height="${monthBandH}" fill="#f1f5f9" stroke="#cbd5e1"/><text x="${curX + (curSpanDays * dayW) / 2}" y="${monthBandH / 2 + 4}" font-size="9" font-weight="700" fill="#475569" text-anchor="middle">${curLabel}</text>`;
        }
        curMonth = m;
        curLabel = `${ganttLoc.months[m]} ${d.getFullYear()}`;
        curX = leftW + i * dayW;
        curSpanDays = 1;
      } else {
        curSpanDays++;
    ...
    // Заголовок по дням (числа) — только когда есть место, чтобы не превратиться в кашу
    let dayTicksHtml = '';
    if (showDayTicks) {
      daysArr.forEach((d, i) => {
        const x = leftW + i * dayW;
        const isWeekend = d.getDay() === 0 || d.getDay() === 6;
        dayTicksHtml += `<rect x="${x}" y="${monthBandH}" width="${dayW}" height="${headerH - monthBandH}" fill="${isWeekend ? '#f1f5f9' : '#ffffff'}" stroke="#e2e8f0"/><text x="${x + dayW / 2}" y="${monthBandH + (headerH - monthBandH) / 2 + 4}" font-size="7" fill="#64748b" text-anchor="middle">${d.getDate()}</text>`;
      });
    }

    // Строки: подпись работы слева + фон-полоса + бар + количество людей рядом с баром
    let rowsHtml = '';
    worksList.forEach((w, idx) => {
      const y = headerH + idx * rowH;
      const isCritical = !!criticalPath[w.id];
      const s = parseDateString(w.start_date);
    ...
    // Связи между работами (та же схема elbow-коннекторов, что и в интерактивной диаграмме)
    let linksHtml = '';
    (dependencies || []).forEach(dep => {
      const pred = worksSchedule[dep.predecessor_id];
      const succ = worksSchedule[dep.successor_id];
      if (!pred || !succ || rowY[pred.id] === undefined || rowY[succ.id] === undefined) return;
      const predStart = parseDateString(pred.start_date);
      const succStart = parseDateString(succ.start_date);
      if (!predStart || !succStart) return;
      const predStartX = leftW + dayOffset(predStart) * dayW;
      const predEndX = predStartX + Math.max(3, (parseInt(pred.duration_days, 10) || 1) * dayW);
      const succStartX = leftW + dayOffset(succStart) * dayW;
      const succEndX = succStartX + Math.max(3, (parseInt(succ.duration_days, 10) || 1) * dayW);

      let x1, x2;
      if (dep.type === 'SS') { x1 = predStartX; x2 = succStartX; }
      else if (dep.type === 'FF') { x1 = predEndX; x2 = succEndX; }
      else if (dep.type === 'SF') { x1 = predStartX; x2 = succEndX; }
      else { x1 = predEndX; x2 = succStartX; }

      const y1 = rowY[pred.id];
      const y2 = rowY[succ.id];
      const bothCritical = !!criticalPath[pred.id] && !!criticalPath[succ.id];
      const pathD = getGanttLinkPath(x1, y1, x2, y2, dep.type || 'FS');
      linksHtml += `<path d="${pathD}" fill="none" stroke="${bothCritical ? '#e11d48' : '#94a3b8'}" stroke-width="1.4" marker-end="url(#sched-arrow)"/>`;
    });

    return `
      <svg width="${svgW}" height="${svgH}" viewBox="0 0 ${svgW} ${svgH}" xmlns="http://www.w3.org/2000/svg" style="font-family: 'Inter', Arial, sans-serif; background:#ffffff;">
        <defs>
          <marker id="sched-arrow" viewBox="0 0 10 10" refX="6" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
            <path d="M 0 1.5 L 6 5 L 0 8.5 z" fill="#64748b" />
          </marker>
        </defs>
        <rect x="0" y="0" width="${svgW}" height="${svgH}" fill="#ffffff" stroke="#cbd5e1"/>
        ${monthsHtml}
        ${dayTicksHtml}
        <rect x="0" y="0" width="${leftW}" height="${headerH}" fill="#f1f5f9" stroke="#cbd5e1"/>
        ${rowsHtml}
        ${linksHtml}
      </svg>
    `;
  ...
  // Экспорт КП в PDF: шапка (проект/объект/зона/фаза) + таблица работ + статическая диаграмма
  // Ганта со связями (US: печатная форма ГПР, перенесена сюда из отдельной вкладки
  // «Распределение объемов», которую убрали за ненадобностью).
  const handleExportSchedulePDF = () => {
    const worksList = Object.values(worksSchedule).sort((a, b) => {
    ...
    worksList.forEach(w => {
      const isCritical = !!criticalPath[w.id];
      const endDateStr = w.start_date ? calculateEndDate(w.start_date, w.duration_days) : '';
      const manager = estimatorsList.find(m => m.id === w.assignee_id);
      const managerName = manager ? `${manager.first_name || ''} ${manager.last_name || ''}`.trim() || manager.email?.split('@')[0] : '—';
      rowsHtml += `
        <tr>
          <td>${w.code || '—'}</td>
    ...
    const objName = objectNameOverride || projectData?.objectName || currentObj?.name || '—';
    const estTypeLabel = docMeta?.estimate_type === 'planned' ? (t.schedTypePlanned || 'Плановая смета') : (t.schedTypeActual || 'Фактическая смета');
    const metaBits = [
      `${t.lblProject || 'Проект'}: <strong>${projectNameOverride || projectData?.name || '—'}</strong>`,
      `${t.gprObject || 'Объект'}: <strong>${objName}</strong>`,
      docMeta?.zone ? `${t.gprLblZone || 'Зона'}: <strong>${docMeta.zone}</strong>` : null,
      docMeta?.phase ? `${t.gprLblPhase || 'Фаза'}: <strong>${docMeta.phase}</strong>` : null,
      `${t.schedEstType || 'Тип сметы'}: <strong>${estTypeLabel}</strong>`,
      `${t.schedStatus || 'Статус'}: <strong>${docMeta ? getStatusLabel(docMeta.status) : '—'}</strong>`
    ].filter(Boolean).join(' &nbsp;•&nbsp; ');

    const ganttSvg = buildStaticGanttSvg(worksList);

    const element = document.createElement('div');
    element.innerHTML = `
      <div style="font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #1e293b; font-size: 10px; line-height: 1.4; padding: 20px; background: #ffffff;">
        <div style="margin-bottom: 20px; border-bottom: 2px solid #e2e8f0; padding-bottom: 12px;">
          <h1 style="font-size: 18px; margin: 0 0 12px 0; font-weight: 800; color: #0f172a;">${t.schedTitle || 'Календарное планирование'}</h1>
          <div style="font-size: 11px; color: #475569;">${metaBits}</div>
        </div>
        ...
        <table style="width: 100%; table-layout: fixed; border-collapse: collapse; margin-top: 10px;">
          ...
          <thead>
            <tr style="background: #f1f5f9;">
              <th style="font-size: 8px; font-weight: 700; text-transform: uppercase; padding: 6px; border: 1px solid #cbd5e1; color: #475569; overflow-wrap: break-word; word-break: break-word;">${t.gprLblCode || 'Код'}</th>
              <th style="font-size: 8px; font-weight: 700; text-transform: uppercase; padding: 6px; border: 1px solid #cbd5e1; color: #475569; overflow-wrap: break-word; word-break: break-word; text-align: left;">${t.gprColWork || 'Работа'}</th>
              <th style="font-size: 8px; font-weight: 700; text-transform: uppercase; padding: 6px; border: 1px solid #cbd5e1; color: #475569; overflow-wrap: break-word; word-break: break-word;">${t.gprColUnit || 'Ед. изм.'}</th>
              <th style="font-size: 8px; font-weight: 700; text-transform: uppercase; padding: 6px; border: 1px solid #cbd5e1; color: #475569; overflow-wrap: break-word; word-break: break-word; text-align: right;">${t.gprColVolume || 'Объем'}</th>
              <th style="font-size: 8px; font-weight: 700; text-transform: uppercase; padding: 6px; border: 1px solid #cbd5e1; color: #475569; overflow-wrap: break-word; word-break: break-word;">${t.gprColStart || 'Начало'}</th>
              <th style="font-size: 8px; font-weight: 700; text-transform: uppercase; padding: 6px; border: 1px solid #cbd5e1; color: #475569; overflow-wrap: break-word; word-break: break-word;">${t.gprColEnd || 'Окончание'}</th>
              <th style="font-size: 8px; font-weight: 700; text-transform: uppercase; padding: 6px; border: 1px solid #cbd5e1; color: #475569; overflow-wrap: break-word; word-break: break-word;">${t.gprColDuration || 'Срок'}</th>
              <th style="font-size: 8px; font-weight: 700; text-transform: uppercase; padding: 6px; border: 1px solid #cbd5e1; color: #475569; overflow-wrap: break-word; word-break: break-word;">${t.schedColLaborCount || 'Людей'}</th>
              <th style="font-size: 8px; font-weight: 700; text-transform: uppercase; padding: 6px; border: 1px solid #cbd5e1; color: #475569; overflow-wrap: break-word; word-break: break-word; text-align: left;">${t.schedColAssigneeFull || 'Ответственный'}</th>
              <th style="font-size: 8px; font-weight: 700; text-transform: uppercase; padding: 6px; border: 1px solid #cbd5e1; color: #475569; overflow-wrap: break-word; word-break: break-word;">${t.gprLblCriticalPath || 'Критический путь'}</th>
            </tr>
          </thead>
          <tbody>
            ${rowsHtml}
        ...
        <div id="sched-gantt-export-slot" style="overflow: visible;">${ganttSvg}</div>
    ...
    // A4-альбомная страница ≈ 297мм, за вычетом полей margin:10мм с каждой стороны —
    // печатная область ≈ 277мм ≈ 1046px при 96dpi. Контейнер шире этого — html2pdf не
    // масштабирует контент под ширину страницы, а обрезает то, что не влезло, поэтому
    // держим ширину контейнера строго внутри печатной области.
    const EXPORT_WIDTH = 1040;
    const opt = {
      margin:       10,
      filename:     `KP_${objName}_${dateStr}.pdf`,
      image:        { type: 'jpeg', quality: 0.98 },
      html2canvas:  { scale: 2, useCORS: true, logging: false, backgroundColor: '#ffffff', windowWidth: EXPORT_WIDTH },
      jsPDF:        { unit: 'mm', format: 'a4', orientation: 'landscape' }
    };

    // ВАЖНО: html2canvas реально рендерит DOM по вычисленным стилям — если элемент невидим
    // (opacity: 0, visibility: hidden) или вынесен далеко за пределы окна (position: fixed/absolute
    // с большим отрицательным left), он честно захватывает пустоту, а не «то, что было бы видно».
    // Проверено эмпирически: только элемент в обычном потоке документа (position: static) и
    // полностью непрозрачный даёт полный снимок. Поэтому прячем не сам элемент, а весь экран —
    // временным белым оверлеем поверх приложения, внутри которого элемент рендерится по-настоящему.
    const overlay = document.createElement('div');
    overlay.style.cssText = 'position: fixed; inset: 0; background: #ffffff; z-index: 999998; overflow: auto; display: flex; flex-direction: column; align-items: center;';
    const overlayLabel = document.createElement('div');
    overlayLabel.textContent = t.gprGeneratingPdf || 'Формируем PDF…';
    overlayLabel.style.cssText = 'padding: 16px; font-family: Inter, sans-serif; font-size: 13px; font-weight: 800; color: #2563eb;';
    ...
    // Даём браузеру отрисовать вставленный DOM (включая SVG) перед снимком.
  ...
  useImperativeHandle(ref, () => ({
    exportPDF: handleExportSchedulePDF,
    hasWorks: () => Object.keys(worksSchedule).length > 0
  }));
      ...
      return (
        <div key={node.id} style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{
            ...
            <div style={{ color: '#cbd5e1', fontWeight: 'bold', textAlign: 'center' }}>—</div>
      ...
      <div style={{
        ...
        <div style={{ maxHeight: '350px', overflowY: 'auto', position: 'relative' }}>
          <div style={{
            ...
            <div style={{ textAlign: 'center' }}>{t.schedColLaborCount || 'Людей'}</div>
    ...
    const renderGanttLeftTree = (node, depth = 0) => {
      ...
      return (
        <div key={`left-gantt-${node.id}`} style={{ display: 'flex', flexDirection: 'column' }}>
          <div style={{
            display: 'grid', gridTemplateColumns: '160px 80px 40px 80px 115px',
                        ...
                        {w.labor_count > 0 && (
                          <div
                            ...
                            style={{
                              ...
                              padding: '1px 6px', fontSize: '10px', fontWeight: '800', color: '#334155',
      ...
      <div style={{
        ...
        <div style={{ maxHeight: compact ? '350px' : '650px', overflow: 'auto', position: 'relative' }}>
          <div style={{ display: 'flex', minWidth: 'fit-content' }}>
            {/* Левая сплит-панель */}
            <div style={{
              width: '475px', minWidth: '475px', borderRight: '1px solid #cbd5e1', flexShrink: 0,
              ...
              <div style={{
                ...
                <div>{t.schedColAssigneeGantt || 'Ответств.'}</div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              {availableWorks.length > 0 && !docMeta?.is_readonly && (
  ...
  const getDocVersionLabel = (doc) => {
    ...
    if (type === 'planned') {
      const verStr = doc.plan_version ? ` ${doc.plan_version}` : '';
  ...
  const getStatusLabel = (status) => {
    if (!status) return '';
    const labels = {
      draft: t.schedStatusDraft || 'Черновик',
      planned_formed: t.schedStatusPlannedFormed || 'Сформирована',
      planned_review: t.schedStatusPlannedReview || 'На утверждении',
      planned_approved: t.schedStatusPlannedApproved || 'Утверждена',
      planned_rejected: t.schedStatusPlannedRejected || 'Отклонена',
      planned_inactive: t.schedStatusPlannedInactive || 'Деактивирована',
      actual_formed: t.schedStatusActualFormed || 'Сформирована'
    };
    return labels[status] || status;
  };

  const getDocVersionBadgeColors = (doc) => {
    const type = doc?.estimate_type || 'work';
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <h2 style={{ margin: 0, fontSize: '18px', fontWeight: '900', color: '#0f172a', letterSpacing: '-0.3px', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            <span style={{ background: 'linear-gradient(135deg, #1e293b, #3b82f6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
              {t.schedTitle || 'Планирование'}
            </span>
            {projectData && (
              <span style={{ fontSize: '13px', fontWeight: '700', color: '#475569', background: '#f1f5f9', padding: '4px 10px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                {t.lblProject || 'Проект'}: {projectData.name || '—'} ({formatDisplayDate(projectStartDate)} — {formatDisplayDate(projectEndDate)})
              </span>
            )}
            {docMeta && (
              <>
                <span style={{
                  ...
                  borderRadius: '8px',
                  whiteSpace: 'nowrap',
                  background: getDocVersionBadgeColors(docMeta).bg,
                  color: getDocVersionBadgeColors(docMeta).color
                }}>
                  {getDocVersionLabel(docMeta)}
                </span>
                <span style={{
                  fontSize: '12px',
                  fontWeight: '800',
                  ...
                  {t.schedStatus || 'Статус'}: {getStatusLabel(docMeta.status)}
                </span>
                <span style={{
                  fontSize: '12px',
                  fontWeight: '800',
                  ...
                  borderRadius: '8px',
                  whiteSpace: 'nowrap',
                  background: docMeta.is_readonly ? '#fef2f2' : '#f0fdf4',
                  color: docMeta.is_readonly ? '#b91c1c' : '#16a34a',
                  border: docMeta.is_readonly ? '1px solid #fca5a5' : '1px solid #bbf7d0',
                  display: 'inline-flex',
                  alignItems: 'center',
                  gap: '4px'
                }}>
                  {docMeta.is_readonly ? (
                    <>🔒 {t.schedEditingDisabled || 'Редактирование запрещено'}</>
                  ) : (
                    <>✏️ {t.schedEditingAllowed || 'Редактирование разрешено'}</>
                  )}
                </span>
              </>
            )}
...
export default forwardRef(SchedulingTab);
