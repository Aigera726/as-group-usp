  ...
  if (editingProject) {
    ...
    return (
      <div className="animate-fade-in" style={{ padding: '40px', maxWidth: '1000px', margin: '0 auto' }}>
        {/* Back Button & Title — переключатель языка тут убрали: он уже есть в шапке ERP
            (RU/EN/KA/AZ сверху), дублировать его внутри модуля незачем */}
                  ...
                  {loadingObjects ? (
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                        ...
                        {/* Кнопка "Открыть WBS" отключена по просьбе — оставлена закомментированной
...
;
  ...
  if (selectedObject) {
    ...
    return (
      <div className="animate-fade-in" style={{ padding: isEstOpen ? '0px' : '24px 40px 40px', maxWidth: isEstOpen ? '100%' : '1700px', margin: '0 auto' }}>
        <ObjectDetailsView
  ...
  return (
    <div className="animate-fade-in" style={{ padding: isEstimateOpen ? '0px' : '24px 40px 40px', maxWidth: isEstimateOpen ? '100%' : '1700px', margin: '0 auto' }}>
      {!isEstimateOpen && (
        <>
          {/* HEADER */}
          ...
          <div style={{ display: 'flex', gap: '4px', borderBottom: '1px solid #e2e8f0', marginBottom: '24px' }}>
            {[
              { id: 'dictionaries', label: t.tabDictionaries, icon: <Layers size={16} />, visible: userRole !== 'financial_director' && userRole !== 'site_manager' },
              { id: 'estimates', label: t.tabEstimates, icon: <Calculator size={16} />, visible: userRole !== 'pricer' && userRole !== 'financial_director' && userRole !== 'site_manager' },
              { id: 'all_estimates', label: t.tabAllEstimates || 'Сметы', icon: <FileText size={16} />, visible: userRole !== 'pricer' && userRole !== 'site_manager' },
              { id: 'gpr', label: t.tabGpr || 'ГПР', icon: <Table size={16} />, visible: userRole !== 'pricer' && userRole !== 'financial_director' && userRole !== 'site_manager' },
              { id: 'gpm', label: t.gpmTitle || 'ГПМ', icon: <Truck size={16} />, visible: userRole !== 'pricer' && userRole !== 'financial_director' },
              { id: 'contracts', label: t.tabContracts || 'Договора', icon: <Briefcase size={16} />, visible: userRole !== 'pricer' && userRole !== 'financial_director' && userRole !== 'site_manager' },
              { id: 'operfacts', label: t.tabOperFacts || 'Оперфакт', icon: <ClipboardCheck size={16} />, visible: userRole !== 'pricer' && userRole !== 'financial_director' && userRole !== 'site_manager' },
            ]
